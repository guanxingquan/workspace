...
#             Config().writeToConfig(Constants.deviceFrameRate,"frame-rate",frame_rate_in_ds)
            ...
            if frame_rate_in_ds <= max and frame_rate_in_ds >= min:
                ...
                log.info("the frame rate from DS is %s, expected rate is %s ,  False",frame_rate_in_ds,rated_frames)
                Config().writeToConfig(Constants.deviceFrameRate,"frame-rate",rated_frames)
                return False  
        except Exception,e:
